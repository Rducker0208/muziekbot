...
class PicksongEmbed(discord.ui.View):
    ...
    async def create_embed(self):
        ...
        for song in self.song_ids:  # verander eerder verzamelde ids in namen
            ...
            self.song_options.append(song_name)
            print(song_name)
...
def main():
    ...
    async def mute(ctx):
        ...
        if first_run is False:
            ...
            print(voice.source)
            voice.source = discord.PCMVolumeTransformer(voice.source, volume=0)
            print(voice.source)
            first_run = False
        await ctx.send(f'Muted multibot')
    ...
    async def qeue_add(ctx, *, qeue_url):  # voeg een url toe aan qeue
        ...
        if qeue_url[:32] == 'https://www.youtube.com/watch?v=':
            ...
            if ctx.voice_client is None:
                await join(ctx)
            await ctx.send('Fetching results, please wait for the embed with options to spawn to avoid errors.')
            qeue_len = len(qeue)
            print(qeue_len)
